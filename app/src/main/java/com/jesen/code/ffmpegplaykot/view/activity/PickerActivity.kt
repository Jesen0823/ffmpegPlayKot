...
import com.jesen.code.ffmpegplaykot.util.extend.otherwise
...
class PickerActivity : AppCompatActivity() {
    ...
    private fun initScanEnvironment() {
        checkPermission(storagePermissions).yes {
            startScan()
        }
    }
    ...
    private fun checkPermission(permissions: Array<String>): Boolean {
        ...
        for (permission in permissions) {
            if (ContextCompat.checkSelfPermission(
                ...
                (ActivityCompat.shouldShowRequestPermissionRationale(this, permission)).yes {
    ...
    override fun onRequestPermissionsResult(
        ...
        when (requestCode) {
            permissionRequestStorage ->
                (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_DENIED).yes {
                    startScan()
                }.otherwise {
